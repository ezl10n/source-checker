...
public class TempletConfigModule extends AbstractModule{
	...
	protected void configure() {
		 bind(Config.class).annotatedWith(Names.named("templetConfig")).toInstance(loadConfig());
		
	}
	@Provides
    public static Config loadConfig(){
        String preFix=String.format("..%1$s%1$ssrc%1$smain%1$sconfig",File.separator);
        String fileName=String.format("%1$sproject-templet-config.conf",File.separator);
